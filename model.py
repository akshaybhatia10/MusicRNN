...
def model_placeholders(input_size, output_size):
	model_inputs = tf.placeholder(tf.float32, shape=(None, timestamp, input_size), name='input_placeholder')
...
def model_parameters(hidden_size, output_size):
...
def rnn_layer(inputs, parameters, rnn_units, keep_prob):
	inputs, tf.unstack(inputs, timestep)
	...
	pred = tf.nn.softmax(final_outputs)
...
def get_loss(final_outputs, targets):
	loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits
						  (logits=final_outputs, labels=targets))		
